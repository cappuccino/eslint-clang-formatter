 ...
 * if chalk is enabled.
 * @param {boolean} colorize - Whether to colorize the text.
 * @param {object} colorMap - Maps style names to colors.
 * @param {string} color - Key in colorMap.
 * @param {string} text - Text to colorize.
 * @returns {string} - A colorized version of text if colorize is true and chalk is enabled.
 */
...
module.exports = function(results, config)
{
    ...
    results.forEach(function(result)
    {
        ...
        messages.forEach(function(message)
        {
            ...
            if (source)
            {
                var prefix = message.source.substr(0, message.column - 1).replace(/./g, " ");

                output += "\n" + source + "\n" + prefix + colorizeText(colorize, colorMap, "caret", "^");
            }
